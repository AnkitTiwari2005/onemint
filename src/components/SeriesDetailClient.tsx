...
/** Minimal article shape needed by this component */
...
interface Props {
  series: Series;
  /** Article data for each slug in the series, fetched server-side from DB */
  seriesArticles: SeriesArticleItem[];
}
...
export function SeriesDetailClient({ series: s, seriesArticles }: Props) {
  ...
  return (
    ...
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 24px 80px' }}>
      ...
      {/* Progress bar */}
      {totalRead > 0 && seriesArticles.length > 0 && (
      ...
      {seriesArticles.length > 0 ? (
        ...
        <div style={{ borderTop: '1px solid var(--color-border)', paddingTop: 32, textAlign: 'center', color: 'var(--color-ink-tertiary)', fontFamily: 'var(--font-ui)', fontSize: 14 }}>
          <p>Articles for this series are being published. Check back soon.</p>
        </div>
      )}
